...
16. Alerts — ⬜ Not started. Watchlists (spec §3, §14) are 🟡 partial: a single saved "discovery preferences" record (make/model/price/mileage/year) is stored in the `watchlists` table and applied at discovery time (`PUT /api/preferences`, `apps/web/src/components/PreferencesPanel.tsx`) so a vehicle outside your taste never enters the pipeline — verified against the live Auto.dev API that `vehicle.make`, `vehicle.model`, `retailListing.price`, and `retailListing.miles` filters all apply correctly. Multiple named watchlists and actual alerts/notifications on top of this aren't built.
