...
12. Maintenance engine — ⬜ Not started. The remaining blocker before `opportunity_score` can be computed at all.
13. Opportunity scoring — 🟡 Partial. The weighting function (`scoreOpportunity`) is implemented and tested against pre-computed 0-100 sub-scores. Market price advantage (step 11) now computes for real; the other six factors don't exist yet, so `opportunity_score` stays unset and nothing appears in the ranked "Opportunities" list.
