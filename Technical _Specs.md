...
6. React dashboard — 🟡 Partial. Two working sections (`apps/web`): a ranked "Opportunities" table with each vehicle's actual photo, a color-coded score tier badge (Exceptional/Excellent/Strong/Fair/Weak per spec §18), and a below/above-market chip — filterable by make/price/mileage. Clicking a row expands a detail view with the full labeled score breakdown (each factor's weight and point contribution), a plain-language "why this price" comparison, dealer info, listing link, and the history evidence log. Plus a "Needs Review" queue with surfaced history-report links and an interactive verification form. No watchlists yet.
...
16. Alerts — ⬜ Not started. Watchlists (spec §3, §14) are ✅ done: full CRUD over up to 3 named saved searches (make/model/price/mileage/year) via `/api/watchlists` (`apps/worker/src/api/watchlists.ts`), each applied as its own discovery pass so a vehicle outside every saved search never enters the pipeline — the dashboard's `WatchlistManager` shows every saved search's filter chips without needing to open an edit form. Verified end-to-end against the live Auto.dev API: cap enforcement, create/update/delete, and that a run with 2 watchlists issues one discovery call per watchlist and returns only matching makes. Actual alerts/notifications on top of this aren't built.
